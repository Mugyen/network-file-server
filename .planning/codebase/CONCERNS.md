...
Technical debt, known issues, and areas of concern for the WiFi File Server codebase.

## Security Concerns

### Hardcoded Secret Key (Critical)
- **File:** `wifi_file_server.py:17`
...
- **File:** `wifi_file_server.py:103-134`
...
- **File:** `wifi_file_server.py:57-82`
...
- **Files:** `wifi_file_server.py:64-74` and `wifi_file_server.py:143-152`
- **Issue:** `index()` and `api_files()` have identical file listing logic copy-pasted
- **Impact:** Bug fixes need to be applied in two places

### Unused Entry Point
- **File:** `main.py`
- **Issue:** `main.py` just prints "Hello from wifi-ftp-server!" — not connected to actual server
...
### IP Detection
- **File:** `wifi_file_server.py:22-30`
...
- **File:** `wifi_file_server.py:197-203`
