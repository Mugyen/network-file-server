...
## 2026-06-11: Production HTTPS deployment on GCP VM (Caddy + nip.io) and secure-context fixes

Relay now runs as a hardened systemd service on the GCP VM behind Caddy TLS (https://34.30.19.224.nip.io until the real domain lands): relay binds 127.0.0.1 only, Caddy terminates TLS with auto-renewed Let's Encrypt certs + HTTP→HTTPS 308, RELAY_ENV=production with strict RELAY_ALLOWED_ORIGINS, persistent state in ~/relay-data, stable session secret. systemd ExecStart uses scripts/run.sh relay so restart = full deploy (uv sync + client rebuild when stale). Verified: health/assets over HTTPS, WS upgrades (browser /m/{code}/ws and /agent/ws) 101 through Caddy via HTTP/1.1, direct :8001 unreachable. Fixed two secure-context bugs the HTTP-on-public-IP interlude exposed: getDeviceId crashed the whole app where crypto.randomUUID is absent (new generateUuid fallback via getRandomValues, websocket.ts), and ShareDialog's unguarded navigator.clipboard.writeText (new shared utils/copyToClipboard.ts, also adopted by SnippetCard). New docs/deployment.md: 3-context HTTPS architecture, DNS-01 plan for LAN TLS, secure-context API review policy. 113 vitest green; tsc/eslint clean.

## 2026-06-11: Relay public_url config — drop box QR advertises the public URL

The drop box QR/server-info showed http://<relay-internal-ip>:0 because init_dropbox passed relay_url=None to the embedded ServerConfig (server_info's mount branch needs mount_code AND relay_url). Added RelayConfig.public_url (env RELAY_PUBLIC_URL > yaml public_url > first allowed origin > None), threaded through lifespan → init_dropbox → ServerConfig.relay_url; dev without origins keeps the local-IP fallback. Set on the VM; live /m/dropbox/api/server-info now returns https://34.30.19.224.nip.io/m/dropbox. +6 tests (config precedence ×4, dropbox server-info ×2); ruff/mypy clean.
