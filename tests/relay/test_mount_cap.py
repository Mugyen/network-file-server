...
def _prefill_mounts(registry: MountRegistry, count: int, agent_ip: str) -> None:
...
async def test_5_mounts_from_same_ip_all_succeed() -> None:
    ...
    _prefill_mounts(registry, 4, _TEST_IP)
...
async def test_6th_mount_from_same_ip_rejected() -> None:
    ...
    app = _make_cap_app(5)
    registry = get_registry()
    _prefill_mounts(registry, 5, _TEST_IP)
...
async def test_expired_mount_frees_cap_slot() -> None:
    ...
    # Expire one mount
    registry._mounts["prefill-0"].status = MountStatus.EXPIRED
